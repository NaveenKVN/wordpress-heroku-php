...
class TouchsizeLikes {
	...
	function like_this($post_id, $zero_postfix = false, $one_postfix = false, $more_postfix = false, $action = 'get'){
		...
		$general_icon = get_option('shootback_general', array('like_icons' => 'heart'));		
		...
		switch($action) {
			...
			case 'update':
				...
				if( isset($_COOKIE['touchsize_likes_'. $post_id]) ) return '<span class="touchsize-likes-count">' . $likes . '</span>';
...
function touchsize_likes($post_id, $before = '', $after = '', $echo = true){
	global $touchsize_likes;
    
    if( $echo ){
    	echo ts_var_sanitize($touchsize_likes->do_likes($post_id, $before, $after)); 
    }else{
    	return ts_var_sanitize($touchsize_likes->do_likes($post_id, $before, $after)); 
    }
}
