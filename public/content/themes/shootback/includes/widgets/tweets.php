<?php
    class widget_tweets extends WP_Widget {
        ...
        function widget_tweets() {
            ...
            parent::__construct( 'widget_touchsize_tweets' ,  __('Latest tweets','shootback') , $widget_ops );
        }
        ...
        ?>
         <p>
          <label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title' , 'shootback' ); ?>:</label>
          <input class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" type="text" value="<?php echo $title; ?>" />
        </p>
        <p>
          <label for="<?php echo $this->get_field_id( 'username' ); ?>"><?php _e( 'Twitter User Name' , 'shootback' ); ?>:</label>
          <input class="widefat" id="<?php echo $this->get_field_id( 'username' ); ?>" name="<?php echo $this->get_field_name( 'username' ); ?>" type="text" value="<?php echo $username; ?>" />
        </p>
        <p>
          <label for="<?php echo $this->get_field_id( 'number' ); ?>"><?php _e( 'Number of latest tweets to show' , 'shootback' ); ?>:</label>
        ...
        <p>
        	<label for="<?php echo $this->get_field_id( 'dynamic' ); ?>"><?php _e( 'Animated' , 'shootback' ); ?>:</label>
        ...
        <p>
        	<label for="<?php echo $this->get_field_id( 'followus' ); ?>"><?php _e( 'Show follow us' , 'shootback' ); ?>:</label>
        ...
        <p>
        	<span class="hint"><?php echo sprintf(__( 'Now you need to do it to create an application in %s https://dev.twitter.com/apps %s and fill the requirements there. Once finished you will have your consumer key, consumer secret, access token and access token secret.' , 'shootback' ), '<a href="https://dev.twitter.com/apps">', '</a>' ); ?></span>

        </p>

		<p>
	        <label for="<?php echo $this->get_field_id( 'consumerKey' ); ?>"><?php _e( 'Consumer key' , 'shootback' ); ?>:</label>
		...
		<p>
	        <label for="<?php echo $this->get_field_id( 'consumerSecret' ); ?>"><?php _e( 'Consumer secret' , 'shootback' ); ?>:</label>
		...
		<p>
	        <label for="<?php echo $this->get_field_id( 'accessToken' ); ?>"><?php _e( 'Access token' , 'shootback' ); ?>:</label>
        ...
        <p>
	        <label for="<?php echo $this->get_field_id( 'accessTokenSecret' ); ?>"><?php _e( 'Access token secret' , 'shootback' ); ?>:</label>
        ...
        function widget( $args, $instance ) {
            ...
            echo ts_var_sanitize($args['before_widget']);

            if ( ! empty( $title ) ) {
                echo ts_var_sanitize($args['before_title'] . $title . $args['after_title']);
            }
            ...
            if ( isset($timelines) && is_array($timelines) && !empty($timelines) ) {
                ...
                if($followus != 'no'){
                    $followus_html =    '<div class="twitter-follow">
                                            ...
                                            <a href="http://twitter.com/' . $username . '" target="_blank">
                                                ' . __('Follow us', 'shootback') . ' <b>@' . $username . '</b>
                                            </a>
                                        </div>';
                ...
                echo ts_var_sanitize($html);

            } else {
                _e( 'Error fetching feeds. Please verify the Twitter settings in the widget.', 'shootback' );
            }

            echo ts_var_sanitize($args['after_widget']);
        }
        ...
        function tweet_time( $time ) {
            ...
            if ( is_numeric( $difference ) && $difference > 0 ) {
                ...
                if ( $difference < 3 ) {
                    return __( 'right now', 'shootback' );
                }

                // If less than minute.
                if ( $difference < $minute ) {
                    return floor( $difference ) . ' ' . __( 'seconds ago', 'shootback' );;
                }

                // If less than 2 minutes.
                if ( $difference < $minute * 2 ) {
                    return __( 'about 1 minute ago', 'shootback' );
                }

                // If less than hour.
                if ( $difference < $hour ) {
                    return floor( $difference / $minute ) . ' ' . __( 'minutes ago', 'shootback' );
                }

                // If less than 2 hours.
                if ( $difference < $hour * 2 ) {
                    return __( 'about 1 hour ago', 'shootback' );
                }

                // If less than day.
                if ( $difference < $day ) {
                    return floor( $difference / $hour ) . ' ' . __( 'hours ago', 'shootback' );
                }

                // If more than day, but less than 2 days.
                if ( $difference > $day && $difference < $day * 2 ) {
                    return __( 'yesterday', 'shootback' );;
                }

                // If less than year.
                if ( $difference < $day * 365 ) {
                    return floor( $difference / $day ) . ' ' . __( 'days ago', 'shootback' );
                }

                // Else return more than a year.
                return __( 'over a year ago', 'shootback' );
            }
        }
