...
class widget_instagram extends WP_Widget {
	...
	function widget_instagram() {
        $options = array( 'classname' => 'instagram_widget', 'description' => __('Instagram Widget' , 'shootback' ) );
        parent::__construct( 'widget_touchsize_instagram' , __( 'Instagram Widget' , 'shootback' )  , $options );
		...
		if(!empty($results['data'])){
			foreach($results['data'] as $item){
				if($picture_number == 0){
					...
					echo sprintf(__('%s Please set the Number of images to show within the widget %s','shootback'),'<strong>', '</strong>');
					break;
				}
				...
				if($show_likes){
					if(!empty($item['likes']['count'])){
						echo "<p class='instagram_likes'>".__('Likes:','shootback')." <span class='likes_count'>".$item['likes']['count']."</span></p>";
					}
				}
				if($show_caption){
			...
			echo "<strong>".__('The user currently does not have any images...','shootback')."</strong>";			
		}
	...
	function form( $instance ) {
		if ( $instance ) {
			...
			$title = __( 'Title', 'shootback' );
			$username = __( 'Username', 'shootback' );
			$access_token = __( 'Access Token', 'shootback' );
			$user_id = __( 'User ID', 'shootback' );
			$picture_size = 'thumbnail';
			$picture_number = 0;
			$show_likes = false;
			$show_caption = false;
		...
		<p>
		<label for="<?php echo $this->get_field_id('user_id'); ?>"><?php _e('User ID:','shootback'); ?></label> 
		...
		<p>
		<label for="<?php echo $this->get_field_id('access_token'); ?>"><?php _e('Access Token:','shootback'); ?></label> 
		...
		<p>
			<label for="<?php echo $this->get_field_id('picture_number'); ?>"><?php _e('Number of Images:','shootback'); ?></label> 
			<select id="<?php echo $this->get_field_id('picture_number'); ?>" name="<?php echo $this->get_field_name('picture_number'); ?>">
					<option value="0"><?php _e('Select Number','shootback'); ?></option>
				<?php for($i=1;$i<11;$i++):?>
		...
		<p>
			<label for="<?php echo $this->get_field_id('picture_size'); ?>"><?php _e('Picture Size:','shootback'); ?></label> 
		...
		<p>
		<label for="<?php echo $this->get_field_id('link_images'); ?>"><?php _e('Link images to full image:','shootback'); ?></label> 
		...
		<p>
		<label for="<?php echo $this->get_field_id('show_likes'); ?>"><?php _e('Show Likes:','shootback'); ?></label> 
		...
		<p>
		<label for="<?php echo $this->get_field_id('show_caption'); ?>"><?php _e('Show Caption:','shootback'); ?></label> 
		...
		<p>
		<label for="<?php echo $this->get_field_id('debug_mode'); ?>"><?php _e('Debug Mode:','shootback'); ?></label> 
		...
		<?php
			$token_msg = sprintf(__('If you do not have an ID or access token, please visit %s Get Access token %s to receive a valid token','shootback'),'<a href="'.$token_url.'" target="_blank">', '</a>');
		?>
		<p><?php echo $token_msg; ?></p>
		<p><?php _e("If you don't know your user id:", 'shootback'); ?> <a href="http://jelled.com/instagram/lookup-user-id" target="_blank"><?php _e('Get you user id', 'shootback'); ?></a></p>
		<?php 
	}
