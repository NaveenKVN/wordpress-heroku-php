...
class widget_flickr extends WP_Widget {
    function widget_flickr() {
        ...
        /* Constructor */
        $widget_ops = array('classname' => 'widget_flickr_photos', 'description' => __( 'Flickr Photos' , 'shootback' ) );
        parent::__construct('widget_touchsize_flickrwidget', __( 'Flickr Photos' , 'shootback' ), $widget_ops);
    }
    ...
    function widget($args, $instance) {
        ...
        $title = empty($instance['title']) ? __('Photo Gallery','shootback') : apply_filters('widget_title', $instance['title']);
...
?>
        <div class="flickr clearfix">
            <script type="text/javascript" src="http://www.flickr.com/badge_code_v2.gne?count=<?php echo ts_var_sanitize($number); ?>&amp;display=<?php echo ts_var_sanitize($showing); ?>&amp;size=s&amp;layout=x&amp;source=user&amp;user=<?php echo ts_var_sanitize($id); ?>"></script>
        <div class="clear"></div>
        </div>
<?php
...
?>
        ...
        <p>
            <label for="<?php echo $this->get_field_id('number'); ?>"><?php _e('Number of photos','shootback') ?>:
                <input class="widefat" id="<?php echo $this->get_field_id('number'); ?>" name="<?php echo $this->get_field_name('number'); ?>" type="text" value="<?php echo esc_attr($number); ?>" />
            </label>
        </p>

        <p>
            <label for="<?php echo $this->get_field_id('showing'); ?>"><?php _e('Showing Method','shootback') ?>:
                <select size="1" name="<?php echo $this->get_field_name('showing'); ?>">
                    <option value="random"<?php if(esc_attr($showing) =='random'){echo 'selected';}?>><?php _e('Random Photo','shootback')?></option>
                    <option value="latest"<?php if(esc_attr($showing) =='latest'){echo 'selected';}?>><?php _e('Latest Photo','shootback') ?></option>
                </select>
            </label>
        </p>
<?php
