<?php
    class widget_comments extends WP_Widget {
        ...
        function widget_comments() {
            $widget_ops = array( 'classname' => 'widget_tabber' , 'description' => __( 'Comments' , 'shootback' ) );
            parent::__construct( 'widget_touchsize_comments' ,  __( 'Comments' , 'shootback' ) , $widget_ops );
        }
        ...
        ?>
            ...
            <div class="widget-container">
                                ...
                                <article class="row">
                                    <div class="col-lg-4 col-md-4 col-sm-4 col-xs-4">
                                        <a class="entry-img" href="<?php echo get_permalink( $comment -> comment_post_ID ) . '#comment-' . $comment -> comment_ID; ?>">
                                            <?php echo get_avatar( $comment, 'thumbnail' , DEFAULT_AVATAR );  ?>
                                        </a>
                                    </div>
                                    <div class="col-lg-8 col-md-8 col-sm-8 col-xs-8">
                                        ...
                                        <div class="widget-meta st">
                                            <ul>
                                                <li class="author"><?php echo ts_var_sanitize($author_url); ?></li>
                                                    <?php
                                                        $comment_time = date_i18n(get_option( 'date_format' ), strtotime($comment->comment_date) );
                                                    ?>
                                                <li class="time">
                                                    <time datetime="<?php echo date('Y-m-d',strtotime($comment -> comment_date)); ?>"><?php echo ts_var_sanitize($comment_time); ?></time>
                                                </li>
                                            </ul>
                                        </div>
                                    </div>
                ...
                <?php
                        ...
                        echo '<p>' . __( 'There are no comments' , 'shootback' ) . '</p>';
                    }
                ?>
                </div>
            <?php
    ...
    ?>
            ...
            <p>
                <label for="<?php echo $this->get_field_id('nr_comments'); ?>"><?php _e( 'Number of comments' , 'shootback' ); ?>:
