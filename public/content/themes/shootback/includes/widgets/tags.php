<?php
    class widget_tags extends WP_Widget {
        ...
        function widget_tags() {
            $widget_ops = array( 'classname' => 'widget_tabber' , 'description' => __( 'Tags' , 'shootback' ) );
            parent::__construct( 'widget_touchsize_tags' ,  __( 'Tags' , 'shootback' ) , $widget_ops );
        }
                ...
                <?php
                    ...
                    if( !empty( $tags ) && is_array( $tags ) ){
                        foreach( $tags as $tag ){
                            ...
                            ?><a class="tag" href="<?php echo esc_url($tag_link) ?>"> <?php echo ts_var_sanitize($tag->name, 'esc_attr'); ?></a><?php
                        }
                    }else{
                        echo '<p>' . __( 'There are no tags.' , 'shootback' ) . '</p>';
                    }
                ?>
            </div>
        <?php
    ...
    ?>
			...
			<p>
                <label for="<?php echo $this->get_field_id('nr_tags'); ?>"><?php _e( 'Number of tags' , 'shootback' ); ?>:
					...
					<span class="hint"><?php _e('Leave blank to show all tags','shootback' ) ?></span>
                </label>
            </p>
    <?php
        }
