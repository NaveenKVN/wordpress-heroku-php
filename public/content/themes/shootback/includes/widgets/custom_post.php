<?php
    class widget_custom_post extends WP_Widget{
        ...
        function widget_custom_post() {
            $widget_ops = array( 'classname' => 'widget_custom_post' , 'description' => __( " Latest custom posts" , 'shootback' ) );
            parent::__construct( 'widget_touchsize_custom_post' , __( " Latest custom posts" , 'shootback' ) , $widget_ops );
        }
						...
						<article class="row">
                            ...
                            <div class="col-lg-8 col-sm-8">
                                <h4 class="">
									...
									<?php
										echo ts_var_sanitize($post->post_title, 'esc_attr');
									?>
									</a>
								</h4>
	                                        ...
	                                        ?>
												<li class="red-comments">
	                                                ...
	                                                <a <?php echo ts_var_sanitize($cnt_a2); ?>>
	                                                    ...
	                                                    <?php
	                                                            echo ts_var_sanitize($post->comment_count) . ' ';
    ...
    ?>
           	...
           	<p>
				<label>
					<?php _e('Select post type: ', 'shootback'); ?>
			            <select class="ts-custom-posts" name="<?php echo $this->get_field_name('customPost'); ?>">
			            	<?php 
			            		if(isset($select_options) && !empty($select_options)) echo $select_options;
			            		else echo '<option>' . __('No custom post type', 'shootback') . '</option>'
			            	?>
						</select>
				</label>
			</p>
			<p>
            	<label>
            		<?php _e('Select taxonomy: ', 'shootback'); ?>
		            <select class="ts-by-taxonomy" name="<?php echo $this->get_field_name('taxonomy'); ?>">
		            	<option <?php selected($taxonomy, '__', true); ?> value="__"><?php _e('Select taxonomy', 'shootback'); ?></option>
			            <option <?php selected($taxonomy, 'post_tag', true); ?> value="post_tag"><?php _e('Post tag', 'shootback'); ?></option>
						<option <?php selected($taxonomy, 'category', true); ?> value="category"><?php _e('Category', 'shootback'); ?></option>
					</select>
				</label>	
            </p>
            ...
            <p class="ts-all-tags" style="display: none;">
            	<label>
            		<?php _e('Select tags: ', 'shootback'); ?>
		            <select name="<?php echo $this->get_field_name('tags'); ?>[]" multiple>
		            	<?php if( isset($all_tags) && is_array($tags) && !empty($all_tags) ) : ?>
		            		<?php foreach($all_tags as $tag) : ?>
		            			<?php if( is_object($tag) ) : ?>
		            				<option <?php if( in_array($tag->slug, $tags) ) echo 'selected="selected"' ?> value="<?php echo $tag->slug; ?>"><?php echo $tag->name ?></option>
		            			<?php endif; ?>
		            		<?php endforeach; ?>
		            	<?php else : ?>
		            		<option value=""><?php _e('No tags', 'shootback'); ?></option>
		            	<?php endif; ?>
					</select>
				</label>	
            </p>
            ...
            <p class="ts-all-category" style="display: none;">
            	<label>
            		<?php _e('Select category: ', 'shootback'); ?>
			...
			<p>
                <label for="<?php echo $this->get_field_id('nr_posts'); ?>"><?php _e( 'Number of posts' , 'shootback' ); ?>:
