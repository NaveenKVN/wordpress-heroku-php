<?php
    class widget_list_events extends WP_Widget {
        ...
        function widget_list_events() {
            $widget_ops = array( 'classname' => 'widget_list_events' , 'description' => __( " List events" , 'shootback' ) );
            parent::__construct( 'widget_touchsize_list_events' , __( "List events" , 'shootback' ) , $widget_ops );
        }
							...
							<article <?php if( $columns == '1' ) echo 'class="row"'; ?>>
                                ...
                                <div class="widget-content-box">
    	                            ...
    	                            <div class="widget-content">
                                        ...
                                        <span class="venue"><?php echo ts_var_sanitize($post_meta['venue'], 'esc_attr'); ?></span>
    ...
    ?>
            <p>
                <label for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php _e('Title','shootback') ?>:
                    <input class="widefat" id="<?php echo esc_attr($this->get_field_id('title')); ?>" name="<?php echo esc_attr($this->get_field_name('title')); ?>" type="text" value="<?php echo esc_attr($title); ?>" />
                </label>
            </p>
            <p>
                <label><?php _e( 'Select event taxonomy:' , 'shootback' ); ?></label>
                <select class="ts-events-widget-taxonomy" name="<?php echo $this->get_field_name('taxonomy'); ?>">
                	<option value="category" <?php selected($taxonomy, 'category'); ?>><?php _e( 'Category' , 'shootback' ); ?></option>
                	<option value="tags" <?php selected($taxonomy, 'tags'); ?>><?php _e( 'Post tag' , 'shootback' ); ?></option>
                </select>
            </p>
            <p class="ts-events-category"> 
            	<label><?php _e('Select categories' , 'shootback'); ?></label>
            	    ...
            	    if ( isset($categories) && is_array($categories) && !empty($categories) ): ?>
            	        <?php foreach ($categories as $index => $category): ?>
            	            <?php if( is_object($category) ) : ?>
            	                <option <?php if( is_array($category_ids) && $taxonomy == 'category' && in_array($category->term_id, $category_ids) ) echo 'selected="selected"' ?> value="<?php echo $category->term_id; ?>"><?php echo $category->cat_name; ?></option>
            	            <?php endif; ?>
            	        <?php endforeach ?>                                    
            	    <?php endif ?>
            	</select>
                <input value="" name="<?php echo $this->get_field_name('category'); ?>" type="hidden" class="ts-category-story">
            </p>
            <p class="ts-events-tags"> 
            	<label><?php _e('Select tags' , 'shootback'); ?></label>
            	<?php $tags = get_tags(); 
            	if( !empty($tags) && is_array($tags) ) : ?>
	            	<select name="ts-events-tags" multiple>
	            	   	<?php foreach($tags as $tag) : ?>
	            	   		<option <?php if( is_array($tags_ids) && $taxonomy == 'tags' && in_array($tag->term_id, $tags_ids) ) echo 'selected="selected"' ?> value="<?php echo $tag->term_id; ?>"><?php echo $tag->name; ?></option>
	            	   	<?php endforeach; ?>
	            	</select>
                    <input value="" name="<?php echo $this->get_field_name('tags'); ?>" type="hidden" class="ts-tags-story">
	            <?php else : ?>
	            	<?php echo __('No tags', 'shootback'); ?>
	            <?php endif; ?>
            </p>
            <script>
            ...
            <p>
                ...
                <select name="<?php echo $this->get_field_name('by_time'); ?>">
                	...
                	<option value="m" <?php selected($by_time, 'm'); ?>><?php _e( 'This month' , 'shootback' ); ?></option>
                	<option value="w" <?php selected($by_time, 'w'); ?>><?php _e( 'This week' , 'shootback' ); ?></option>
                </select>
            </p>
            <p>
            ...
            <p>
                <label for="<?php echo $this->get_field_name('image'); ?>"><?php _e('With image', 'shootback' ); ?></label>
                <select name="<?php echo $this->get_field_name('image'); ?>">
                	<option value="y" <?php selected($image, 'y'); ?>><?php _e( 'Yes' , 'shootback' ); ?></option>
                	<option value="n" <?php selected($image, 'n'); ?>><?php _e( 'No' , 'shootback' ); ?></option>
                </select>
            </p>
    <?php
        }
