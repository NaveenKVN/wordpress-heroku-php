...
{
	$sidebars = update_option( 'shootback_sidebars', $sidebars );
}
...
{
	$sidebars = ts_get_sidebars();
	$options  = '<option value="0">' . __(' -- select sidebar -- ', 'shootback'). '</option>';
	$options  .= '<option value="main" ' . selected( $sidebar_id, 'main', false ) . ' >' . __('Main Sidebar', 'shootback') . '</option>';
	$id       = '';
