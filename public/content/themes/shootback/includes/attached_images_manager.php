...
function red_meta_boxes() {
	...
	foreach($post_types as $post_type){
		if( $post_type == 'ts-gallery' ){
			add_meta_box( 'ts-gallery-images-gallery', __( 'Attached Images', 'shootback' ), 'red_product_images_box', $post_type, 'normal' );
		}else{
			add_meta_box( 'ts-gallery-images', __( 'Attached Images', 'shootback' ), 'red_product_images_box', $post_type, 'normal' );
		}
	}
}
				...
				if ( isset( $attachments ) ){
					foreach ( $attachments as $attachment_id ) {
						...
						<li class="image" data-attachment_id="' . $attachment_id . '">
							...
							<ul class="actions">
								<li><a href="#" class="icon-close" title="' . __( 'Delete image', 'shootback' ) . '"></a></li>
								<li class="ts-edit-image"><a href="#" class="icon-brush" title="' . __( 'Edit image', 'shootback' ) . '"></a></li>
							</ul>
						</li>';
					}
				}
			...
			function ts_edit_image(){
				...
				jQuery(document).on("click", ".ts-edit-image", function(event) {
					...
					product_gallery_frame = wp.media.frames.downloadable_file = wp.media({
						...
						title: '<?php _e( 'Edit images', 'shootback' ); ?>',
						button: {
							text: '<?php _e( 'Save changes', 'shootback' ); ?>',
						},
						multiple: false
					});
							...
							if ( attachment.id ) {
								...
								$el.closest('.image').attr('data-attachment_id', attachment.id).html('\
									...
									<ul class="actions">\
										...
										<li class="ts-edit-image"><a href="#" class="icon-brush" title="<?php _e( 'Edit image', 'shootback' ); ?>"></a></li>\
									</ul>'
								);
							}
						} );
