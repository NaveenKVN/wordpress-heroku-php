				...
				if($depth === 0){
					if( isset($query) && !empty($query)){
						...
						foreach($query as $wp_post){
							...
							$title_post     = $wp_post->post_title;
							$permalink_post = get_permalink($wp_post->ID);

							$article_date =  get_the_date('', $wp_post->ID);
												...
												<article class="title-below-image">
														...
														<div class="image-holder">
															...
															<div class="sierra-effect">
													            <a href="'.$permalink_post.'">
													                <span>'.__('Read more', 'shootback').'</span>
													            </a>
													        </div>
														</div>
													</div>
													<div class="section">
														<div class="entry-title">
															<h3 class="title">
																<a href="' . $permalink_post . '">' . $title_post . '</a>
															</h3>
														</div>
														<div class="ts-date">
															'. $article_date .'
	...
	class ts_backend_walker extends Walker_Nav_Menu
	{
		...
		function start_el(&$output, $item, $depth = 0, $args = array(), $current_object_id = 0 ) {
			...
			if ( isset( $item->post_status ) && 'draft' == $item->post_status ) {
				...
				$title = sprintf( __('%s (Pending)', 'shootback'), $item->title );
			}
			...
			if( $item->object === 'category' || $item->object === 'videos_categories' || $item->object === 'portfolio_register_post_type' || $item->object === 'teams'  || $item->object === 'gallery_categories' ){
				$item->category_posts = get_post_meta($item->ID, '_menu-item-category-posts', true);
				$item->nr_of_columns  = get_post_meta($item->ID, '_menu-item-nr-of-columns', true);
			}
			...
			<li id="menu-item-<?php echo $item_id; ?>" class="<?php echo esc_attr($itemValue); echo implode(' ', $classes ); ?>">
				<dl class="menu-item-bar">
					<dt class="menu-item-handle">
						<span class="item-title"><?php echo esc_html( $title ); ?></span>
						<span class="item-controls">
							<span class="item-type item-type-default"><?php echo esc_html( $item->type_label ); ?></span>
							<span class="item-type item-type-ts"><?php _e('-- Is Column -- ','shootback'); ?></span>
							<span class="item-type item-type-megafied"><?php _e('-- Is Mega Menu --','shootback'); ?></span>
							<span class="item-order">
								...
								?>" class="item-move-up"><abbr title="<?php esc_attr_e('Move up','shootback'); ?>">&#8593;</abbr></a>
								|
								<a href="<?php
									echo wp_nonce_url(
										add_query_arg(
								...
								?>" class="item-move-down"><abbr title="<?php esc_attr_e('Move down','shootback'); ?>">&#8595;</abbr></a>
							</span>
							<a class="item-edit" id="edit-<?php echo $item_id; ?>" title="<?php _e('Edit Menu Item','shootback'); ?>" href="<?php
							...
							?>"><?php _e( 'Edit Menu Item','shootback' ); ?></a>
						</span>
					</dt>
				</dl>
				...
				<div class="menu-item-settings" id="menu-item-settings-<?php echo $item_id; ?>">
					...
					<p class="description description-thin description-label ts_label_desc_on_active">
						...
						<span class='ts_default_label'><?php _e( 'Navigation Label','shootback' ); ?></span>
						<span class='ts_is_mega_label'><?php _e( 'Mega Menu Column Title <span class="ts_supersmall">(if you dont want to display a title just enter a single dash: "-" )</span>','shootback' ); ?></span>
					...
					<p class="description description-thin description-title">
						<label for="edit-menu-item-attr-title-<?php echo $item_id; ?>">
							<?php _e( 'Title Attribute','shootback' ); ?><br />
							<input type="text" id="edit-menu-item-attr-title-<?php echo $item_id; ?>" class="widefat edit-menu-item-attr-title" name="menu-item-attr-title[<?php echo $item_id; ?>]" value="<?php echo esc_attr( $item->post_excerpt ); ?>" />
						</label>
					</p>
					<p class="field-link-target description description-thin">
						<label for="edit-menu-item-target-<?php echo $item_id; ?>">
							<?php _e( 'Menu Link Target','shootback' ); ?><br />
							<select id="edit-menu-item-target-<?php echo $item_id; ?>" class="widefat edit-menu-item-target" name="menu-item-target[<?php echo $item_id; ?>]">
								<option value="" <?php selected( $item->target, ''); ?>><?php _e('Open in same window','shootback'); ?></option>
								<option value="_blank" <?php selected( $item->target, '_blank'); ?>><?php _e('Open in new window','shootback'); ?></option>
							</select>
						</label>
					</p>
					<p class="field-css-classes description description-thin">
						<label for="edit-menu-item-classes-<?php echo $item_id; ?>">
							<?php _e( 'Custom CSS Classes','shootback' ); ?><br />
							<input type="text" id="edit-menu-item-classes-<?php echo $item_id; ?>" class="widefat code edit-menu-item-classes" name="menu-item-classes[<?php echo $item_id; ?>]" value="<?php echo esc_attr( implode(' ', $item->classes ) ); ?>" />
						</label>
					</p>
					<p class="field-icons description description-icons">
						<label for="edit-menu-item-icon-<?php echo $item_id; ?>">
							<?php _e( 'Add icon' ,'shootback'); ?><br />
						</label>
					...
					<p class="field-description description description-wide">
						<label for="edit-menu-item-description-<?php echo $item_id; ?>">
							<?php _e( 'Description' ,'shootback'); ?><br />
					...
					<?php if( ($item->object === 'category' || $item->object === 'videos_categories' || $item->object === 'portfolio_register_post_type' || $item->object === 'teams' || $item->object === 'gallery_categories') && $item->menu_item_parent == 0 ) : ?>
						<p class="field-category-posts category-posts description-category-posts">
							...
							<select class="ts-menu-category-posts" name="menu-item-category-posts[<?php echo $item_id; ?>]" id="edit-menu-item-category-posts-<?php echo $item_id; ?>">
								<option value="y" <?php selected($item->category_posts, 'y'); ?>><?php _e('Yes', 'shootback'); ?></option>
								<option value="n" <?php selected($item->category_posts, 'n'); ?>><?php _e('No', 'shootback'); ?></option>
							</select>
						...
						<p class="field-nr-of-columns nr-of-columns description-nr-of-columns">
							<label for="edit-menu-item-nr-of-columns-<?php echo $item_id; ?>">
								<?php _e( 'Number of columns:' ,'shootback'); ?><br />
							</label>
							<select name="menu-item-nr-of-columns[<?php echo $item_id; ?>]" id="edit-menu-item-nr-of-columns-<?php echo $item_id; ?>">
								<option value="3" <?php selected($item->nr_of_columns, '3'); ?>><?php _e('3', 'shootback'); ?></option>
								<option value="4" <?php selected($item->nr_of_columns, '4'); ?>><?php _e('4', 'shootback'); ?></option>
								<option value="5" <?php selected($item->nr_of_columns, '5'); ?>><?php _e('5', 'shootback'); ?></option>
							</select>
						</p>
					<?php endif; ?>
					...
					<!-- ################# ts custom code here ################# -->
						...
						<p class="description description-wide ts_checkbox ts_is_mega_menu ts_is_mega_menu_d0">
							<label for="edit-<?php echo $key.'-'.$item_id; ?>">
								<input type="checkbox" value="active" id="edit-<?php echo $key.'-'.$item_id; ?>" class="ts-megamenu-category-posts <?php echo $key; ?>" name="<?php echo $key . "[". $item_id ."]";?>" <?php echo $value; ?> /><?php _e( 'Set as Mega Menu','shootback' ); ?>
							</label>
						</p>
						<!-- ***************  end item *************** -->
						...
						<p class="description description-wide ts_checkbox ts_is_mega_menu ts_is_mega_menu_d1">
							<label for="edit-<?php echo $key.'-'.$item_id; ?>">
								<input type="checkbox" value="active" id="edit-<?php echo $key.'-'.$item_id; ?>" class=" <?php echo $key; ?>" name="<?php echo $key . "[". $item_id ."]";?>" <?php echo $value; ?> /><?php _e('Column starts new row','shootback'); ?>
							</label>
						</p>
						<!-- ***************  end item *************** -->
						...
						<?php
						$title = __('Check to create text content. This will not display the item as a link. (Please dont remove the label text, WordPress will automatically remove the item.)','shootback');
					...
					<div class="menu-item-actions description-wide submitbox">
						<?php if( 'custom' != $item->type ) : ?>
							<p class="link-to-original">
								<?php printf( __('Original: %s', 'shootback'), '<a href="' . esc_attr( $item->url ) . '">' . esc_html( $original_title ) . '</a>' ); ?>
							</p>
						<?php endif; ?>
						...
						); ?>"><?php _e('Remove','shootback'); ?></a> <span class="meta-sep"> | </span> <a class="item-cancel submitcancel" id="cancel-<?php echo $item_id; ?>" href="<?php	echo add_query_arg( array('edit-menu-item' => $item_id, 'cancel' => time()), remove_query_arg( $removed_args, esc_url(admin_url( 'nav-menus.php' )) ) );
...
if( !function_exists( 'ts_fallback_menu' ) )
{
	...
	function ts_fallback_menu()
	{
		...
		echo "<li $current><a href='".home_url()."'>".__('Home','shootback')."</a></li>";
