...
function validate_layout_title()
{
	if ( empty( $_POST['template_name'] ) ) {
		return new WP_Error( 'empty_title', __( "Title is empty", 'shootback' ) );
	} else {
...
<form action="<?php echo admin_url( 'admin.php?page=templates&action=create' ); ?>" method="POST">
	<table cellpadding="10">
		<tr>
			<td><label for="template_name"><?php _e('Template name','shootback'); ?>:</label></td>
			<td><input type="text" name="template_name" id="template_name" /></td>
		</tr>
		<tr>
			<td></td>
			<td><input type="submit" name="create_template" class="button-primary red-ui-button" value="<?php _e('Create Template','shootback');?>" /></td>
		</tr>
	</table>
</form>
<?php } ?>
