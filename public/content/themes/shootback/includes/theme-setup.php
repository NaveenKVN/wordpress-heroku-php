<?php
define('SHOOTBACK_VERSION', time());

function shootback_activation_settings($theme)
{

    $first_activation = get_option('shootback_first_activation', 'yes');
    
    if ( $first_activation === 'yes' ) {

        $header = '[{"settings":{"rowName":"","bgColor":"transparent","textColor":"inherit","rowMaskColor":"inherit","rowMask":"no","rowShadow":"no","textOpacity":"","bgImage":"","bgVideoMp":"","bgVideoWebm":"","bgPosition":"left","bgAttachement":"fixed","bgRepeat":"repeat","bgSize":"auto","rowMarginTop":"0","rowMarginBottom":"40","rowPaddingTop":"40","rowPaddingBottom":"40","expandRow":"no","specialEffects":"none","rowTextAlign":"auto","fullscreenRow":"no","rowVerticalAlign":"top","scrollDownButton":"no","rowOpacity":0},"columns":[{"size":4,"elements":[{"type":"logo","logo-align":"text-left"}]},{"size":8,"elements":[{"type":"menu","element-style":"style1","menu-custom":"no","menu-bg-color":"#DDDDDD","menu-text-color":"#FFFFFF","menu-bg-color-hover":"#DDDDDD","menu-text-color-hover":"#FFFFFF","submenu-bg-color":"#DDDDDD","submenu-text-color":"#FFFFFF","submenu-bg-color-hover":"#DDDDDD","submenu-text-color-hover":"#FFFFFF","menu-text-align":"menu-text-align-left","admin-label":"","uppercase":"menu-no-uppercase"}]}]}]';

        $header = json_decode($header, true);

        $header_templates = array(
            'default' => array(
                'name' => __('Default template', 'shootback'),
                'elements' => $header
            )
        );

        update_option( 'shootback_header', $header );
        update_option( 'shootback_header_template_id', 'default' );
        update_option( 'shootback_header_templates', $header_templates );

        $footer = '[{"settings":{"rowName":"","bgColor":"#f6f6f6","textColor":"inherit","rowMaskColor":"inherit","rowMask":"no","rowShadow":"no","textOpacity":"","bgImage":"","bgVideoMp":"","bgVideoWebm":"","bgPosition":"left","bgAttachement":"fixed","bgRepeat":"repeat","bgSize":"auto","rowMarginTop":"40","rowMarginBottom":"0","rowPaddingTop":"40","rowPaddingBottom":"40","expandRow":"no","specialEffects":"none","rowTextAlign":"auto","fullscreenRow":"no","rowVerticalAlign":"top","rowOpacity":0},"columns":[{"size":12,"elements":[{"type":"text","text":"Copyright TouchSize. All rights reserved. www.touchsize.com<\/p>","admin-label":""}]}]}]';
        ...
        update_option( 'shootback_footer', $footer );
        update_option( 'shootback_footer_template_id', 'default' );
        update_option( 'shootback_footer_templates', $footer_templates );

        update_option('shootback_first_activation', 'no');
    }
}


add_action('after_switch_theme', 'shootback_activation_settings');

if ( function_exists( 'add_theme_support' ) ) {

	/*
	 * Makes Shootback available for translation.
	 *
	 * Translations can be added to the /languages/ directory.
	 */
	load_theme_textdomain( 'shootback', get_template_directory() . '/languages' );
...
function register_shootback_menu() {
	register_nav_menus( array(
		'primary' => __( 'Primary' , 'shootback' )
	));
}

add_action( 'init', 'register_shootback_menu' );
...
if ( ! function_exists( 'red_get_post_img_slideshow' ) ) { 
    function red_get_post_img_slideshow($post_id, $size="grid"){
        ...
        ?>                                 
                <li>
                    <img src="<?php echo $src; ?>"  data-original="<?php echo $src; ?>" alt="" data-width="<?php echo esc_attr($thumbnail_url[1]); ?>" data-height="<?php echo esc_attr($thumbnail_url[2]); ?>" />                               
                    <?php if ( ts_overlay_effect_is_enabled() ): ?>
                        <div class="dotted"></div>
                    <?php endif ?>
                    <a class="zoom-in-icon" href="<?php echo $src; ?>" rel="fancybox-[<?php echo $post_id; ?>]"><i class="icon-search"></i>
                        <img class="hidden" src="<?php echo $src; ?>" alt="" />
                    </a>
                </li>
...
add_action('media_buttons_context',  'shootback_toggle_editor');

function shootback_toggle_editor() {
    ...
    if (isset($post)) {
        ...
        if ($post->post_type === 'page') {
            ...
            $button = '<div class="icon-blocks" id="ts-toggle-layout-builder" data-state="'.$builder_status.'">' . __('Toggle Layout Builder', 'shootback') . '</div>';
?>
            <script>
...
/* Localization */
load_theme_textdomain( 'shootback' );
load_theme_textdomain( 'shootback' , get_template_directory() . '/languages' );

if ( function_exists( 'load_child_theme_textdomain' ) ){
    load_child_theme_textdomain( 'shootback' );
}
