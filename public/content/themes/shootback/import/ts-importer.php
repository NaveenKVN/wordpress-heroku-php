<?php
 ...
 if ( !class_exists( 'Ts_Importer' ) ) {
	...
	class Ts_Importer {
		...
		public $options_txt  = 'settings.txt';
		public $optionsDemo;
    	...
    	public function intro_html() {
			...
			<div style="background-color: #F5FAFD; margin:10px 0;padding: 5px 10px;color: #0C518F;border: 2px solid #CAE0F3; clear:both; width:90%; line-height:18px;">
			    ...
			    allow you to quickly edit everything instead of creating content from scratch. When you import the data following things will happen:', 'shootback'); ?></p>

			      <ul style="padding-left: 20px;list-style-position: inside;list-style-type: square;}">
			          <li><?php _e('No existing posts, pages, categories, images, custom post types or any other data will be deleted or modified .', 'shootback'); ?></li>
			          <li><?php _e('No WordPress settings will be modified .', 'shootback'); ?></li>
			          <li><?php _e('Posts, pages, some images, some widgets and menus will get imported .', 'shootback'); ?></li>
			          <li><?php _e('Images will be downloaded from our server, these images are copyrighted and are for demo use only .', 'shootback'); ?></li>
			          <li><?php _e('Please click import only once and wait, it can take a couple of minutes', 'shootback'); ?></li>
			      </ul>
			 </div>
			<?php if( !empty($this->imported_demos) ) { ?>
			  	...
			  	<div style="background-color: #FAFFFB; margin:10px 0;padding: 5px 10px;color: #8AB38A;border: 2px solid #a1d3a2; clear:both; width:90%; line-height:18px;">
			  		<p><?php _e('Demo already imported', 'shootback'); ?></p>
			  	</div><?php
			   	//return;
	    ...
	    public function demo_installer() {
			...
			if( !empty($this->imported_demos ) ) {
				...
				$button_text = __('Import Again', 'shootback');

			} else {

				$button_text = __('Import Demo Data', 'shootback');

			}

	        ?>
	        <h2><?php _e('Import Demo Data', 'touchsize'); ?></h2>
	       ...
	       <div class="ts-importer-wrap" data-demo-id="1"  data-nonce="<?php echo wp_create_nonce('ts-demo-code'); ?>">
		        ...
		        <form method="post">
		        	<?php $this->intro_html(); ?>
		          	...
		          	<input name="reset" class="panel-save button-primary ts-import-start" type="submit" value="<?php echo $button_text ; ?>" />
        ...
        public function ajax_import_widget_data(){
        	...
        	$json_data = file_get_contents($this->widgets);
        ...
        public static function parse_import_data( $import_array ) {
        	...
        	$currentSidebarsTs = get_option('shootback_sidebars');
        ...
        public function set_demo_options(){
        	...
        	if($file_headers[0] !== 'HTTP/1.1 404 Not Found') {
        		...
        		$options = ts_base_64($file_data, 'decode');
        		...
        		echo __('File settings.txt not found', 'shootback');
        	}
