jQuery(document).ready(function($) {
    ...
    var shootback_custom_uploader = {};

    $(document).on("click", ".shootback_select_image", function(e) {
        e.preventDefault();
        ...
        if (shootback_custom_uploader[target_id]) {
            shootback_custom_uploader[target_id].open();
            return;
        }

        //Extend the wp.media object
        shootback_custom_uploader[target_id] = wp.media.frames.file_frame[target_id] = wp.media({
            title: 'Choose Image',
            button: {
                text: 'Choose Image'
            },
        ...
        shootback_custom_uploader[target_id].on('select', function() {

            var attachment = shootback_custom_uploader[target_id].state().get('selection').first().toJSON();
            ...
            if (target_id === 'upload-logo') {
				...
				$(newImg).load(function(){
					$('#shootback_logo_retina_width').val(newImg.width);
					$('#shootback_logo_retina_height').val(newImg.height);
				});
			}
	...
	$(document).on('click', '.ts-remove-alert', function(event) {
		...
		data['action'] = 'shootback_hide_touchsize_alert';
		data['token'] = token;
	...
	jQuery('#generate-likes').click(function(){
		...
		jQuery.post(ajaxurl, "action=ts_generate_like&nonce=" + ShootbackAdmin.LikeGenerate, function(response){
			if( response === '1' ){
	...
	jQuery('.meta_title_single_layout ul.imageRadioMetaUl li img').click(function(){
		var layoutSelected = jQuery(this).attr('data-value');
		if( layoutSelected == 'gallery-horizontal' || layoutSelected == 'gallery-horizontal-scroll' ){
			jQuery('.meta_title_gallery_title_position').css('display', 'none');
		}else{
			jQuery('.meta_title_gallery_title_position').css('display', '');
		}
	});

	var layoutSelected = jQuery('.meta_title_single_layout ul.imageRadioMetaUl li img.selected').attr('data-value');
	if( layoutSelected == 'gallery-horizontal' || layoutSelected == 'gallery-horizontal-scroll' ){
		jQuery('.meta_title_gallery_title_position').css('display', 'none');
	}else{
		jQuery('.meta_title_gallery_title_position').css('display', '');
...
jQuery(document).ready( function() {
	...
	jQuery(".featured").click(function() {
		...
		var nonce_featured = ShootbackAdmin.Nonce;
